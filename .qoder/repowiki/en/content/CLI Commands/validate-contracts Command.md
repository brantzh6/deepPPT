...
- Reference slide extractions: validates reference_slide_extraction.schema.json against multiple curated examples.
- Pattern cards: validates pattern_card.schema.json against curated pattern artifacts.
- Benchmark gallery: validates benchmark_gallery.schema.json against a curated gallery artifact.

**Section sources**
- [validateContracts.ts:7-24](file://src/commands/validateContracts.ts#L7-L24)
- [validateContracts.ts:26-83](file://src/commands/validateContracts.ts#L26-L83)
- [validateContracts.ts:85-98](file://src/commands/validateContracts.ts#L85-L98)
...
- Reference Slide Extraction
  ...
  - Examples: reference_slide_extraction.example.json and several curated reference slide artifacts.
...
- Pattern Card
  ...
  - Examples: curated pattern artifacts in style/patterns.
...
**Section sources**
- [validateContracts.ts:87-96](file://src/commands/validateContracts.ts#L87-L96)

## Conclusion
The validate-contracts command establishes a reliable checkpoint for data integrity by validating curated examples and artifacts against their schemas. Its integration with the schema catalog and AJV 2020 ensures consistent, comprehensive validation with clear error reporting. By catching schema violations early, it improves the reliability of downstream pipeline stages and reduces the cost of fixing data issues later in the process.